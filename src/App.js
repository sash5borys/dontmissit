import React, { useContext } from 'react';
import { StateContext } from './data/StateProvider';
import Modal from './components/Modal';
import ServiceBlock from './components/ServiceBlock';

const App = () => {
  const { state } = useContext(StateContext);
  const ws = new WebSocket('ws://127.0.0.1:8080');
  const stateServices = Object.keys(state.services);

  return (
    <div className="app">
      <div className="container header-container">
        <header>
          <a className="app-logo">
            <h1 className="app-logo__title">D!</h1>
            <img
              className="app-logo__img"
              src="data:image/png;base64,"
            />
          </a>
          <nav>
            <ul className="app-srv__list center-col">
              {/*foreach*/}
              {/*}

              <li className="app-srv__list__item"><button style={{backgroundImage:`url("https://img.icons8.com/color/96/undefined/facebook.png")`}}></button></li>
              <li className="app-srv__list__item"><button style={{backgroundImage:`url("https://img.icons8.com/color/96/undefined/twitter-squared.png")`}}></button></li>
              
              {*/}
              {/*endforeach*/}
              <li className="app-srv__list__item"><button className="app-srv__list__add-button">+</button></li>

            </ul>
          </nav>
        </header>
      </div>
      <div className="container full-container">
        <Modal />
        <section className={'app-srv-block ' + (stateServices.length < 3 && 'center-container')}>
          {true &&
            stateServices.map((service) => {
              return <ServiceBlock key={service} serviceName={service} ws={ws} />;
            })}


            <section className="app-srv-block__list">
              <form className="app-srv-block__list__add-form">
                <h2>Додай сторінку <br /> для відслідковування</h2>
                <div className="app-srv-block__list__add-form_add-button">
                  <input type="text" />
                  <button type="submit">
                    +
                  </button>
                </div>
                <ul className="app-srv__list center-row">
                  {/*foreach*/}
                  <li className="app-srv__list__item"><button style={{backgroundImage:`url("https://img.icons8.com/color/96/undefined/facebook.png")`}}></button></li>
                  <li className="app-srv__list__item"><button style={{backgroundImage:`url("https://img.icons8.com/color/96/undefined/twitter-squared.png")`}}></button></li>
                  {/*endforeach*/}
                </ul>
              </form>
            </section>


        </section>
      </div>
      {/*<div className="container">
        <footer>
          <span>Слава Україні!</span>
          <span className="app-flag"></span>
        </footer>
      </div>*/}
    </div>
  );
};

export default App;
